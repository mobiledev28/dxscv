...
var ScriptDynamic = {
    ...
    SearchMarkers: function (status, fechafinal, fechainicial, ciudad, estado, tecnico, vendedor, cliente, magnitud) {
        //Quita del mapa los marcadores de la busqueda anterior
        ScriptDynamic.SetMapOnAll(null);
        //Obtiene los marcadores de la nueva busqueda
        $.ajax({
            beforeSend: function () {
                $('.loading').show();
            },
            ...
            success: function (data) {
                ...
                if (data.success == "yes") {
                    for (var i = 0; i <= data.data.length - 1; i++) {
                        if (data.data[i].Longitude != 0 && data.data[i].Latitude != 0) {

                            ScriptDynamic.AddMarkers(data.data[i]);
                            //$("#hdnStatus").val(data.data[i].Status);
                            $("#hdnCliente").val(data.data[i].Cliente);
                        }
                    }
